...
"out vec4 color; \n"
"\n"
"uniform sampler2D screenSprite; \n"
"\n"
"void main()\n"
"{\n"
"    vec4 sampled = vec4(texture(screenSprite, TexCoords).r, texture(screenSprite, TexCoords).g, texture(screenSprite, TexCoords).b, texture(screenSprite, TexCoords).a);\n"
"    color = sampled;\n"
"}\0";
...
void RenderSystem::updateScreenSpace() {	
	initScreenSpace();

	// setup fonts
	std::string font_filename = font_path("Neucha-Regular.ttf");
...
void RenderSystem::initializeGlMeshes()
{
	for (uint i = 0; i < mesh_paths.size(); i++)
	{
		...
		std::cout << "path of mesh file is " << name << std::endl;
...
bool RenderSystem::fontInit(const std::string& font_filename, unsigned int font_default_size) {
	...
	glShaderSource(font_vertexShader, 1, &screenVertexShaderSource, NULL);
	...
	float wwidth = cam.dims.x, wheight = cam.dims.y;
...
bool RenderSystem::initScreenSpace() {

	// font buffer setup
	glGenVertexArrays(1, &m_screen_VAO);
	glGenBuffers(1, &m_screen_VBO);

	unsigned int screen_vertexShader;
	screen_vertexShader = glCreateShader(GL_VERTEX_SHADER);
	glShaderSource(screen_vertexShader, 1, &screenVertexShaderSource, NULL);
	glCompileShader(screen_vertexShader);

	unsigned int screen_fragmentShader;
	screen_fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(screen_fragmentShader, 1, &screenFragmentShaderSource, NULL);
	glCompileShader(screen_fragmentShader);

	m_screen_shaderProgram = glCreateProgram();
	glAttachShader(m_screen_shaderProgram, screen_vertexShader);
	glAttachShader(m_screen_shaderProgram, screen_fragmentShader);
	glLinkProgram(m_screen_shaderProgram);
	...
	glm::mat4 projection = glm::ortho(0.0f, static_cast<float>(wwidth), 0.0f, static_cast<float>(wheight));
	GLint screen_location = glGetUniformLocation(m_screen_shaderProgram, "projection");
	assert(screen_location > -1);
	std::cout << "project_location: " << screen_location << std::endl;
	glUniformMatrix4fv(screen_location, 1, GL_FALSE, glm::value_ptr(projection));
